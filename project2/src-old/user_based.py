...
def predict_ratings(train_ratings, test_ratings, \
	...
	def calculate_rating(num_users, ratings, similarities, \
		...
		# Determine K nearest neighbours for the given user
		K = 50  #TODO Pass as argument
		have_rated_item = np.nonzero(ratings[item, :])[0]

		top_K_similarities = []
		for neighbour in have_rated_item:

			top_K_similarities.append((neighbour, \
									   abs(similarities[user, neighbour])))


		top_K_similarities.sort(key = lambda tup: tup[1], reverse=True)
		top_K_similarities = top_K_similarities[:K]
		
		# Calculate rating
		numerator = 0
		denominator = 0
		for neighbour, sim in top_K_similarities:
