...
def init_MF(train, num_features):
    ...
    print(len(item_nnz), len(item_sum))
...
def create_submission(path, predictions, submission_data):
...
test_err, W, Z = ALS(train, test, 10, best_config[1]/1000, best_config[2]/1000)
best_W_Z = (W,Z)

X = best_W_Z[1].transpose().dot(best_W_Z[0])
# create_submission("./data/ULTI.csv", X)
create_submission("./data/test_set_predictions.csv", X, test)